...
theorem mem_prefixBarycenterSegment_of_mem_ambientCoordinateFace_of_lineMap_stdSimplexVertex
    {n k : ℕ} [NeZero k] (hk : k + 1 ≤ n) {y z : RentCoordinates n} {c : ℝ}
    (hyFace : y ∈ ambientCoordinateFace (prefixRooms n (k + 1)))
    (hc0 : 0 < c) (hc1 : c < 1)
    (hline :
      let ik : RoomIndex n := ⟨k, lt_of_lt_of_le (Nat.lt_succ_self k) hk⟩
      z = AffineMap.lineMap y
        (((stdSimplex.vertex (S := ℝ) ik : RentSimplex n) : RentCoordinates n)) c)
  ...
  have hprefix :
      ∀ {i : RoomIndex n}, i.1 < k → y i = (k : ℝ)⁻¹ - y ik / k := by
    intro i hi
    have hzprefix : z i = (k : ℝ)⁻¹ - z ik / k := by
      simpa [ik] using mem_prefixBarycenterSegment_apply_of_lt hk hzSeg hi
    have hi_ne_ik : i ≠ ik := by
      intro hii
      have : i.1 = k := by simpa [ik] using congrArg Fin.val hii
      exact (Nat.ne_of_lt hi) this
    have hzi : z i = (1 - c) * y i := by
      rw [hline]
      have hvertex_zero :
          (((stdSimplex.vertex (S := ℝ) ik : RentSimplex n) : RentCoordinates n) i) = 0 := by
        simp [stdSimplex_vertex_apply, hi_ne_ik]
      rw [AffineMap.lineMap_apply_module]
      change (1 - c) * y i +
          c * (((stdSimplex.vertex (S := ℝ) ik : RentSimplex n) : RentCoordinates n) i) =
        (1 - c) * y i
      rw [hvertex_zero]
      ring
    have hzk : z ik = (1 - c) * y ik + c := by
      rw [hline]
      have hvertex_one :
          (((stdSimplex.vertex (S := ℝ) ik : RentSimplex n) : RentCoordinates n) ik) = 1 := by
        simp [stdSimplex_vertex_apply, ik]
      rw [AffineMap.lineMap_apply_module]
      change (1 - c) * y ik +
          c * (((stdSimplex.vertex (S := ℝ) ik : RentSimplex n) : RentCoordinates n) ik) =
        (1 - c) * y ik + c
      rw [hvertex_one]
      ring
    rw [hzi, hzk] at hzprefix
    have hkR : (k : ℝ) ≠ 0 := by
      exact_mod_cast (NeZero.ne k)
    have h1c_ne : (1 - c : ℝ) ≠ 0 := by
      linarith
    field_simp [hkR] at hzprefix
    have hfactor : (1 - c) * ((k : ℝ) * y i + y ik - 1) = 0 := by
      nlinarith [hzprefix]
    have hlinear : (k : ℝ) * y i + y ik - 1 = 0 := by
      rcases mul_eq_zero.mp hfactor with hzero | hzero
      · linarith
      · exact hzero
    have : y i = (k : ℝ)⁻¹ - y ik / k := by
      field_simp [hkR]
      linarith [hlinear]
    exact this
  have hyk_le :
    ...
    have hyk_le_one : y ik ≤ 1 := by
      ...
      calc
        y ik ≤ ∑ j : RoomIndex n, y j := by
          exact Finset.single_le_sum (fun j _ => hyFace.1.1 j) (Finset.mem_univ ik)
        _ = (1 : ℝ) := hsum
    have hzk : z ik = y ik + c * (1 - y ik) := by
      rw [hline]
      simp [AffineMap.lineMap_apply_module, ik]
      ring
    have hyk_le_zk : y ik ≤ z ik := by
      rw [hzk]
      nlinarith
    have hzk_le : z ik ≤ ((k + 1 : ℝ)⁻¹) := by
      simpa [ik] using mem_prefixBarycenterSegment_levelCoord_le hk hzSeg
    exact le_trans hyk_le_zk hzk_le
  exact
    mem_prefixBarycenterSegment_of_mem_ambientCoordinateFace_and_apply_of_lt_and_levelCoord_le
...
theorem ambientCoordinateFace_pair_of_lineMap_mem
    ...
    (hc0 : 0 < c) (hc1 : c < 1)
    (hxyFace : AffineMap.lineMap x y c ∈ ambientCoordinateFace I) :
    x ∈ ambientCoordinateFace I ∧ y ∈ ambientCoordinateFace I := by
  have hzero :
      ∀ i, i ∉ I → x i = 0 ∧ y i = 0 := by
    intro i hi
    have hline_zero : AffineMap.lineMap x y c i = 0 :=
      (coordSupport_subset_iff.mp hxyFace.2) i hi
    have hx_nonneg : 0 ≤ x i := hxSimplex.1 i
    have hy_nonneg : 0 ≤ y i := hySimplex.1 i
    have h1c_pos : 0 < 1 - c := by linarith
    have hx_term_nonneg : 0 ≤ (1 - c) * x i := by positivity
    have hy_term_nonneg : 0 ≤ c * y i := by positivity
    have hsum_zero : (1 - c) * x i + c * y i = 0 := by
      simpa [AffineMap.lineMap_apply_module] using hline_zero
    have hx_term_zero : (1 - c) * x i = 0 := by linarith
    have hy_term_zero : c * y i = 0 := by linarith
    have h1c_ne : 1 - c ≠ 0 := by linarith
    have hc_ne : c ≠ 0 := by linarith
    exact
      ⟨(mul_eq_zero.mp hx_term_zero).resolve_left h1c_ne,
        (mul_eq_zero.mp hy_term_zero).resolve_left hc_ne⟩
  refine ⟨⟨hxSimplex, ?_⟩, ⟨hySimplex, ?_⟩⟩
  · rw [coordSupport_subset_iff]
    intro i hi
    exact (hzero i hi).1
  · rw [coordSupport_subset_iff]
...
theorem ambientCoordinateFace_pair_of_lineMap_mem_prefixBarycenterSegment
    {n k : ℕ} [NeZero k] (hk : k + 1 ≤ n) {x y : RentCoordinates n} {c : ℝ}
    (hxSimplex : x ∈ scaledSimplex 1 n) (hySimplex : y ∈ scaledSimplex 1 n)
    (hc0 : 0 < c) (hc1 : c < 1)
    (hxySeg : AffineMap.lineMap x y c ∈ prefixBarycenterSegment n k) :
    x ∈ ambientCoordinateFace (prefixRooms n (k + 1)) ∧
      y ∈ ambientCoordinateFace (prefixRooms n (k + 1)) := by
  exact ambientCoordinateFace_pair_of_lineMap_mem hxSimplex hySimplex hc0 hc1
    (prefixBarycenterSegment_subset_ambientCoordinateFace hk hxySeg)
...
theorem IsFaceRespecting.exists_erased_face_point_mem_upperPrefixFace_of_minimal_section5SegmentSubface
    {n : ℕ} [NeZero n] {T : SimplexTriangulation n} {f : SelfMapOnRentSimplex n}
    ...
    ∃ x x' : RentSimplex n, ∃ c : ℝ,
      ...
      f x' ∈ ambientCoordinateFace (prefixRooms n (u.level + 1)) := by
  ...
  rcases hfpl.exists_erased_face_point_of_minimal_section5SegmentSubface
    ...
    ⟨x, x', c, hxτ, hfxSeg, hx'erase, hc0, hc1, hx_line, hfx_line⟩
  have hfx'_face :
      f x' ∈ ambientCoordinateFace (prefixRooms n (u.level + 1)) := by
    have hlineSeg :
        AffineMap.lineMap (f x') (f v) c ∈ prefixBarycenterSegment n u.level := by
      simpa [hfx_line] using hfxSeg
    exact
      (ambientCoordinateFace_pair_of_lineMap_mem_prefixBarycenterSegment hu.level_le
        (hf.mem_scaledSimplex x') (hf.mem_scaledSimplex v) hc0 hc1 hlineSeg).1
  exact ⟨x, x', c, hxτ, hfxSeg, hx'erase, hc0, hc1, hx_line, hfx_line, hfx'_face⟩

theorem IsFaceRespecting.exists_erased_face_point_chart_mem_upperPrefixFace_of_minimal_section5SegmentSubface
    {n : ℕ} [NeZero n] {T : SimplexTriangulation n} {f : SelfMapOnRentSimplex n}
    (hf : IsFaceRespecting f) (hfpl : IsPiecewiseAffineOn T f)
    {u : Section5Node n} (hu : IsSection5GraphNode T f u) (hulevel : 0 < u.level)
    {τ : SimplexFacet n} (hτ : τ ∈ section5SegmentSubfaces u f)
    ...
    ∃ x x' : RentSimplex n, ∃ c : ℝ,
      ((x : RentSimplex n) : RentCoordinates n) ∈ τ.realization ∧
      g x ∈ prefixBarycenterSegment n u.level ∧
      ((x' : RentSimplex n) : RentCoordinates n) ∈ (⟨τ.vertices.erase v⟩ : SimplexFacet n).realization ∧
      0 < c ∧ c < 1 ∧
      ((x : RentSimplex n) : RentCoordinates n) =
        AffineMap.lineMap (((x' : RentSimplex n) : RentCoordinates n))
          (((v : RentSimplex n) : RentCoordinates n)) c ∧
      g x = AffineMap.lineMap (g x') (g v) c ∧
      g x' ∈ ambientCoordinateFace (prefixRooms n (u.level + 1)) := by
  rcases hf.exists_erased_face_point_mem_upperPrefixFace_of_minimal_section5SegmentSubface
      hfpl hu hulevel hτ hmin hv hcard with
    ⟨x, x', c, hxτ, hfxSeg, hx'erase, hc0, hc1, hx_line, hfx_line, hfx'_face⟩
  have hx'τ : ((x' : RentSimplex n) : RentCoordinates n) ∈ τ.realization := by
    exact SimplexFacet.realization_mono_of_isSubface
      (by intro w hw; exact Finset.mem_of_mem_erase hw) hx'erase
  refine ⟨x, x', c, hxτ, ?_, hx'erase, hc0, hc1, hx_line, ?_, ?_⟩
  · simpa [hg x hxτ] using hfxSeg
  · rw [hg x hxτ, hg x' hx'τ, hg v]
    · exact hfx_line
    · rw [SimplexFacet.realization]
      exact subset_convexHull ℝ _ <|
        Set.mem_image_of_mem ((↑) : RentSimplex n → RentCoordinates n) hv
  · simpa [hg x' hx'τ] using hfx'_face

theorem
    IsFaceRespecting.exists_erased_face_point_chart_mem_prefixBarycenterSegment_of_minimal_section5SegmentSubface_of_vertexImage_eq_stdSimplexVertex
    {n : ℕ} [NeZero n] {T : SimplexTriangulation n} {f : SelfMapOnRentSimplex n}
    (hf : IsFaceRespecting f) (hfpl : IsPiecewiseAffineOn T f)
    {u : Section5Node n} (hu : IsSection5GraphNode T f u) (hulevel : 0 < u.level)
    {τ : SimplexFacet n} (hτ : τ ∈ section5SegmentSubfaces u f)
    ...
    (hvImage :
      ...
      g v = (((stdSimplex.vertex (S := ℝ) ik : RentSimplex n) : RentCoordinates n))) :
    ∃ x x' : RentSimplex n, ∃ c : ℝ,
      ((x : RentSimplex n) : RentCoordinates n) ∈ τ.realization ∧
      g x ∈ prefixBarycenterSegment n u.level ∧
      ((x' : RentSimplex n) : RentCoordinates n) ∈ (⟨τ.vertices.erase v⟩ : SimplexFacet n).realization ∧
      ...
      ((x : RentSimplex n) : RentCoordinates n) =
        AffineMap.lineMap (((x' : RentSimplex n) : RentCoordinates n))
          (((v : RentSimplex n) : RentCoordinates n)) c ∧
      g x = AffineMap.lineMap (g x') (g v) c ∧
      g x' ∈ prefixBarycenterSegment n u.level := by
  haveI : NeZero u.level := ⟨Nat.ne_of_gt hulevel⟩
  rcases hf.exists_erased_face_point_chart_mem_upperPrefixFace_of_minimal_section5SegmentSubface
      hfpl hu hulevel hτ hmin hg hv hcard with
    ⟨x, x', c, hxτ, hgxSeg, hx'erase, hc0, hc1, hx_line, hgx_line, hgx'_face⟩
  have hgx'_seg : g x' ∈ prefixBarycenterSegment n u.level := by
    have hgx_line' :
        g x = AffineMap.lineMap (g x')
          (((stdSimplex.vertex (S := ℝ)
            (⟨u.level, lt_of_lt_of_le (Nat.lt_succ_self u.level) hu.level_le⟩ :
              RoomIndex n) : RentSimplex n) : RentCoordinates n)) c := by
      simpa [hvImage] using hgx_line
    exact mem_prefixBarycenterSegment_of_mem_ambientCoordinateFace_of_lineMap_stdSimplexVertex
      hu.level_le hgx'_face hc0 hc1 hgx_line' hgxSeg
  exact ⟨x, x', c, hxτ, hgxSeg, hx'erase, hc0, hc1, hx_line, hgx_line, hgx'_seg⟩
...
theorem IsPiecewiseAffineOn.minimal_section5SegmentSubface_mem_coordinateFace_of_erase_vertexImage_eq_stdSimplexVertex
    {n : ℕ} [NeZero n] {T : SimplexTriangulation n} {f : SelfMapOnRentSimplex n}
    (hf : IsFaceRespecting f) (hfpl : IsPiecewiseAffineOn T f)
    {u : Section5Node n} (hu : IsSection5GraphNode T f u) (hulevel : 0 < u.level)
    {τ : SimplexFacet n} (hτ : τ ∈ section5SegmentSubfaces u f)
    ...
    (hvertex :
      ∀ ⦃v : RentSimplex n⦄, v ∈ τ.vertices → v ∉ coordinateFace (prefixRooms n u.level) →
        ...
        g v = (((stdSimplex.vertex (S := ℝ) ik : RentSimplex n) : RentCoordinates n))) :
    x ∈ coordinateFace (prefixRooms n u.level) := by
  apply τ.mem_coordinateFace_of_mem_realization_of_vertices_mem_coordinateFace hxτ
  apply minimal_section5SegmentSubface_vertices_mem_coordinateFace_of_erase_realization_map_segment
    hfpl hu hτ hmin
  intro v hv hvFace
  rcases
      hf.exists_erased_face_point_chart_mem_prefixBarycenterSegment_of_minimal_section5SegmentSubface_of_vertexImage_eq_stdSimplexVertex
        hfpl hu hulevel hτ hmin hg hv hcard (hvertex hv hvFace) with
    ⟨x0, x', c, hx0τ, _hgx0Seg, hx'erase, _hc0, _hc1, _hx0_line, _hgx0_line, hgx'Seg⟩
  have hx'τ : ((x' : RentSimplex n) : RentCoordinates n) ∈ τ.realization :=
    SimplexFacet.realization_mono_of_isSubface
      (by intro w hw; exact Finset.mem_of_mem_erase hw) hx'erase
  refine ⟨x', hx'erase, ?_⟩
  simpa [hg x' hx'τ] using hgx'Seg
...
/-- Canonical lower-entry data on one Section 5 cell: the canonical lower-prefix face has the
right cardinality, and the current slice already contains a point on that lower prefix face. This
is stronger than the minimal-face packages and feeds directly into the lower-step theorem. -/
structure Section5CanonicalLowerEntryData {n : ℕ} (u : Section5Node n)
    (f : SelfMapOnRentSimplex n) where
  card_eq_lowerPrefixVertices : (section5LowerPrefixVertices u).card = u.level
  point : RentSimplex n
  point_mem_slice : point ∈ section5CellSlice u f
  point_mem_coordinateFace : point ∈ coordinateFace (prefixRooms n u.level)

def Section5CanonicalLowerEntryData.toLowerEntryFaceData
    {n : ℕ} [NeZero n] {T : SimplexTriangulation n} {f : SelfMapOnRentSimplex n}
    (hf : IsFaceRespecting f) (hfpl : IsPiecewiseAffineOn T f)
    {u : Section5Node n} (hu : IsSection5GraphNode T f u) (hulevel : 0 < u.level)
    (hdata : Section5CanonicalLowerEntryData u f) :
    Section5LowerEntryFaceData u f := by
  exact section5LowerEntryFaceData_of_card_eq_and_mem_realization_map_segment
    hf hfpl hu hulevel hdata.point_mem_slice.1 hdata.point_mem_coordinateFace
    hdata.point_mem_slice.2 hdata.card_eq_lowerPrefixVertices

theorem section5CanonicalLowerEntryData_nonempty_iff_card_eq_and_facetImageContains_lowerPrefixVertices
    {n : ℕ} [NeZero n] {T : SimplexTriangulation n} {f : SelfMapOnRentSimplex n}
    (hf : IsFaceRespecting f) (hfpl : IsPiecewiseAffineOn T f) {u : Section5Node n}
    (hu : IsSection5GraphNode T f u) (hulevel : 0 < u.level) :
    Nonempty (Section5CanonicalLowerEntryData u f) ↔
      (section5LowerPrefixVertices u).card = u.level ∧
        FacetImageContains f (⟨section5LowerPrefixVertices u⟩ : SimplexFacet n)
          (prefixBarycenter n u.level) := by
  constructor
  · rintro ⟨hdata⟩
    refine ⟨hdata.card_eq_lowerPrefixVertices, ?_⟩
    exact
      (hfpl.facetImageContains_section5LowerPrefixVertices_iff_exists_mem_section5CellSlice_and_mem_coordinateFace
        hf hu hulevel).2
        ⟨hdata.point, hdata.point_mem_slice, hdata.point_mem_coordinateFace⟩
  · rintro ⟨hcard, hhit⟩
    rcases
        hfpl.exists_mem_section5CellSlice_and_mem_coordinateFace_of_facetImageContains_section5LowerPrefixVertices
          hu hhit with
...
theorem section5CanonicalLowerEntryData_nonempty_iff_lowerEntryFaceData_nonempty
    {n : ℕ} [NeZero n] {T : SimplexTriangulation n} {f : SelfMapOnRentSimplex n}
    (hf : IsFaceRespecting f) (hfpl : IsPiecewiseAffineOn T f) {u : Section5Node n}
    (hu : IsSection5GraphNode T f u) (hulevel : 0 < u.level) :
    Nonempty (Section5CanonicalLowerEntryData u f) ↔
      Nonempty (Section5LowerEntryFaceData u f) := by
...
/-- Stronger direct support layer for the manuscript's lower-entry claim: every non-start cell has
the canonical lower-prefix face of size `k` together with an actual slice point on that face.
This bypasses the blocked minimal-face reduction and converts immediately to lower-entry-face and
step data. -/
structure Section5CanonicalLowerEntryGenericity {n : ℕ} [NeZero n]
    (T : SimplexTriangulation n) (f : SelfMapOnRentSimplex n)
    (hstart : IsSection5GraphNode T f (section5StartNode n)) where
  lower_entry_data_of_ne_start :
    ∀ v : section5StartComponent hstart,
      v ≠ section5StartVertexInComponent hstart →
        Section5CanonicalLowerEntryData v.1.1 f
  upper_step_unique :
    ∀ {v u w : section5StartComponent hstart},
      Section5Step f v.1.1 u.1.1 →
      Section5Step f v.1.1 w.1.1 →
...
def Section5CanonicalLowerEntryGenericity.toEntryFaceGenericity {n : ℕ} [NeZero n]
    {T : SimplexTriangulation n} {f : SelfMapOnRentSimplex n}
    (hf : IsFaceRespecting f) (hfpl : IsPiecewiseAffineOn T f)
    {hstart : IsSection5GraphNode T f (section5StartNode n)}
    (hcanon : Section5CanonicalLowerEntryGenericity T f hstart) :
    Section5EntryFaceGenericity T f hstart := by
  refine ⟨?_, hcanon.upper_step_unique, hcanon.no_upper_step_is_endpoint⟩
  intro v hv
  have hv_node : IsSection5GraphNode T f v.1.1 := (mem_section5Nodes_iff).mp v.1.2
  have hv_level : 0 < v.1.1.level := section5StartComponent_pos_level_of_ne_start hv
  exact (hcanon.lower_entry_data_of_ne_start v hv).toLowerEntryFaceData
    hf hfpl hv_node hv_level

def Section5EntryFaceGenericity.toCanonicalLowerEntryGenericity {n : ℕ} [NeZero n]
    {T : SimplexTriangulation n} {f : SelfMapOnRentSimplex n}
    (hf : IsFaceRespecting f) (hfpl : IsPiecewiseAffineOn T f)
    {hstart : IsSection5GraphNode T f (section5StartNode n)}
    (hentry : Section5EntryFaceGenericity T f hstart) :
  ...
  have hnonempty : Nonempty (Section5CanonicalLowerEntryData v.1.1 f) := by
    rw [section5CanonicalLowerEntryData_nonempty_iff_lowerEntryFaceData_nonempty
      hf hfpl hv_node hv_level]
    exact ⟨hentry.lower_entry_face_of_ne_start v hv⟩
...
theorem Section5CanonicalLowerEntryGenericity.toPerturbationGenericity {n : ℕ} [NeZero n]
    {T : SimplexTriangulation n} {f : SelfMapOnRentSimplex n}
    (hf : IsFaceRespecting f) (hfpl : IsPiecewiseAffineOn T f)
    {hstart : IsSection5GraphNode T f (section5StartNode n)}
    (hcanon : Section5CanonicalLowerEntryGenericity T f hstart) :
  ...
  refine ⟨?_, hcanon.upper_step_unique, hcanon.no_upper_step_is_endpoint⟩
  intro v hv
  exact Section5LowerEntryFaceData.exists_startComponentLowerStep hv
    ((hcanon.toEntryFaceGenericity hf hfpl).lower_entry_face_of_ne_start v hv)
...
theorem Section5CanonicalLowerEntryGenericity.toBoundarySegmentGenericity {n : ℕ} [NeZero n]
    {T : SimplexTriangulation n} {f : SelfMapOnRentSimplex n}
    (hf : IsFaceRespecting f) (hfpl : IsPiecewiseAffineOn T f)
    {hstart : IsSection5GraphNode T f (section5StartNode n)}
    (hcanon : Section5CanonicalLowerEntryGenericity T f hstart) :
    Section5BoundarySegmentGenericity T f hstart := by
  exact (hcanon.toPerturbationGenericity hf hfpl).toBoundarySegmentGenericity
...
theorem Section5CanonicalBoundarySuccessorData.exists_targetFacet_of_canonicalLowerEntryGenericity
    {n : ℕ} [NeZero n] {T : SimplexTriangulation n} {f : SelfMapOnRentSimplex n}
    (hf : IsFaceRespecting f) (hfpl : IsPiecewiseAffineOn T f)
    (hsucc : Section5CanonicalBoundarySuccessorData T f hf)
    (hcanon : Section5CanonicalLowerEntryGenericity T f hf.section5StartNode_isGraphNode) :
    ∃ τ ∈ T.facets,
      FacetImageContains f τ ((rentBarycenter n : RentSimplex n) : RentCoordinates n) := by
...
theorem IsFaceRespecting.exists_barycenter_targetFacet_of_two_le_and_canonicalLowerEntryGenericity
    {n : ℕ} [NeZero n] {T : SimplexTriangulation n} {f : SelfMapOnRentSimplex n}
