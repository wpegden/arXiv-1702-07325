...
- [ ] Prove the actual lower-boundary geometry from the manuscript’s segment-intersection picture. The local target is now canonical: for a non-start cell `u`, choose a minimal segment-hitting subface `τ`, produce a slice point `x ∈ τ.realization` with `x ∈ coordinateFace (prefixRooms n u.level)` and `f x ∈ prefixBarycenterSegment n u.level`, and prove `(section5FacetLowerPrefixVertices u τ).card = u.level`. The new theorems `mem_realization_section5FacetLowerPrefixVertices_of_mem_realization_of_mem_coordinateFace` and `minimal_section5SegmentSubface_lowerBoundaryGeometry_of_facetLowerPrefixVertices_card_eq` then turn exactly that point-plus-cardinality input into the codimension-one lower boundary face required for `Section5MinimalSliceLowerBoundaryGeometry u f`, `Section5MinimalSliceFaceData`, and the predecessor `Section5Step`.
