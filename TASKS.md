...
- [ ] Prove the existence half of `Section5CanonicalBoundarySuccessorData` from actual geometry on the boundary edge `[e₁,e₂]`: construct some level-1 successor in the real start component; uniqueness is now automatic from `section5_levelOne_start_subface_unique`.
