...
- [ ] Finish `Section5PerturbationGenericity.lower_step_exists_of_ne_start` by proving the manuscript's actual lower-entry statement for a non-start Section 5 cell `u`: produce a concrete codimension-one lower face `τ ≤ u.cell` with `τ.vertices.card = u.level`, all vertices of `τ` in `coordinateFace (prefixRooms n u.level)`, and a point of `τ.realization` whose image still lies on `prefixBarycenterSegment n u.level`. The new direct theorem `exists_section5StartComponentLowerStep_of_subface_card_eq_and_mem_realization_map_segment` now converts exactly that data into the predecessor `Section5Step`, so the remaining work is to identify the entering face and a segment-point witness on it. The old `section5LowerPrefixVertices` cardinality route remains available only as a fallback.
