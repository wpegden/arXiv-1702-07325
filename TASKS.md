...
- [ ] Prove the actual lower-boundary geometry from the manuscript’s segment-intersection picture. The remaining local target is now sharper: for a non-start cell `u`, choose a minimal segment-hitting subface `τ`, prove from the generic 1-dimensional slice picture that bad vertices admit the erased-face slice witnesses required by the new theorem, and prove the resulting minimal face has `u.level` vertices. Then `Section5MinimalSliceLowerBoundaryGeometry u f`, `Section5MinimalSliceFaceData`, and the predecessor `Section5Step` follow immediately.
