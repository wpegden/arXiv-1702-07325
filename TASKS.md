...
- [x] Normalize upper steps combinatorially: prove a lower cell is the induced prefix face of any supercell, an upper step adds exactly one new vertex, and two upper steps are already equal once they lie in the same incident facet.
- [x] Push the distinct-facet route one step further: generalize the old boundary convex-hull extraction to arbitrary prefix faces via `exists_upper_prefix_vertex_of_mem_convexHull`, and prove `IsSection5GraphNode.upper_step_eq_of_common_point_outside_lowerFace`, which collapses any two upper steps once a common non-lower point on the collapsed slice is available.
- [ ] Prove the first exact missing slice lemma behind `section5LocalOneComplexGeometry_of_uniqueUpperOrEndpoint`: if `u -> v` is an upper `Section5Step`, then for any witness package from `IsSection5GraphNode.exists_point_in_incidentPrefixFace_mem_segmentZeroFiber_of_piecewiseAffineOn` on `u`, the corresponding local slice `prefixSegmentZeroFiber n u.level g` contributes some point of the incident upper prefix face outside `ambientCoordinateFace (prefixRooms n (u.level + 1))`; after transporting that point across `face_intersection`, apply `IsSection5GraphNode.upper_step_eq_of_common_point_outside_lowerFace` to get uniqueness of the upper continuation.
- [ ] Prove the complementary exact missing slice lemma: if for a start-component node `u` every such local slice point stays in `ambientCoordinateFace (prefixRooms n (u.level + 1))`, then `u` already satisfies `IsSection5Endpoint T f u.1.1`.
