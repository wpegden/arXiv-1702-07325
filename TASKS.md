...
- [ ] Extract the actual boundary-face data from the new compact interval `section5HitParams u f`: show the left endpoint lands on a lower codimension-one face, show the right endpoint can land on at most one upper codimension-one face, and show that when there is only one boundary hit the remaining endpoint is `t = 1`.
