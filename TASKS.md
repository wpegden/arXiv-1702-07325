...
- [ ] Finish `Section5PerturbationGenericity.lower_step_exists_of_ne_start`, but no longer by chasing the bare codomain lemma exposed by the minimal-subface erasure package. The new theorem `IsPiecewiseAffineOn.exists_erased_face_point_of_minimal_section5SegmentSubface` gives the exact affine image formula one would want, but `PAPERNOTES.md` now records a concrete counterexample showing that from `lineMap (f x') (f v) c ∈ [b_k,b_{k+1}]` and only the current ambient-face support hypotheses, one cannot conclude `f x' ∈ [b_k,b_{k+1}]`. So the remaining job is to formalize the stronger local transversality/genericity statement the manuscript is actually using on one simplex cell, or to route the proof back through `Section5BoundarySegmentGenericity` as the honest missing input. The immediate next proof task is to identify the minimal additional codomain hypothesis under which the erased-face contradiction becomes valid, and then use that stronger statement to recover the lower predecessor `Section5Step`.
