...
- [ ] Prove the manuscript’s actual lower-entry-face existence on `u.cell`. For each non-start cell `u`, construct a subface `ρ ≤ u.cell` with `ρ.vertices.card = u.level`, every vertex of `ρ` in `coordinateFace (prefixRooms n u.level)`, and `FacetImageContains f ρ (prefixBarycenter n u.level)`. This direct entry-face theorem is now the main lower-step input and bypasses the blocked task of proving that a separately chosen minimal segment-hitting face contains the lower boundary face.
