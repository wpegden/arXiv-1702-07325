...
- [ ] Extract the actual boundary-face data from the new compact interval `section5HitParams u f`: on the left, upgrade the new “all vertices lie in the lower prefix face” statement to the missing codimension-one/cardinality claim `τ.vertices.card = u.level`; on the right, show the endpoint can land on at most one upper codimension-one face, and show that when there is only one boundary hit the remaining endpoint is `t = 1`.
