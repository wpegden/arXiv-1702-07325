...
- [ ] Finish `Section5PerturbationGenericity.lower_step_exists_of_ne_start` by proving the manuscript's actual lower-entry statement for a non-start Section 5 cell `u`. The direct route is now reduced to one explicit codomain lemma on the canonical minimal segment-hitting face: `section5SegmentSubfaces u f` is finite, a minimal-cardinality member `τ` exists, `IsPiecewiseAffineOn.exists_point_with_nonzero_weights_of_minimal_section5SegmentSubface` gives a segment witness with nonzero barycentric weight on every vertex of `τ`, and the new theorem `IsPiecewiseAffineOn.exists_erased_face_point_of_minimal_section5SegmentSubface` packages the one-vertex removal geometry itself. For any chosen `v ∈ τ.vertices` with `1 < τ.vertices.card`, it produces a point `x'` in the erased face realization and a coefficient `0 < c < 1` such that the original witness point is `lineMap x' v c` and its image is `lineMap (f x') (f v) c`. The remaining job is therefore exactly: if `v` lies outside `coordinateFace (prefixRooms n u.level)`, prove from this affine image formula that `f x'` still lies in `prefixBarycenterSegment n u.level`, so the erased face remains in `section5SegmentSubfaces u f`. Then `minimal_section5SegmentSubface_vertices_mem_coordinateFace_of_erase_mem` forces all vertices of `τ` into the lower prefix face, `τ.vertices.card` upgrades to `u.level`, and `exists_section5StartComponentLowerStep_of_subface_card_eq_and_mem_realization_map_segment` yields the predecessor `Section5Step`.
