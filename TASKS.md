...
- [ ] Finish `Section5PerturbationGenericity.lower_step_exists_of_ne_start` by proving the manuscript's actual lower-entry statement for a non-start Section 5 cell `u`: produce a concrete codimension-one lower face `τ ≤ u.cell` with `τ.vertices.card = u.level`, all vertices of `τ` in `coordinateFace (prefixRooms n u.level)`, and `FacetImageContains f τ (prefixBarycenter n u.level)` (or equivalently a lower-face point on `prefixBarycenterSegment n u.level`, which the new endpoint lemma turns into `b_k`). The old `section5LowerPrefixVertices` cardinality route remains available, but is no longer the only path.
