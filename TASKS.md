...
- [ ] Prove the actual lower-boundary geometry from the manuscript’s segment-intersection picture. The remaining local target is now sharper: for a non-start cell `u`, choose a minimal segment-hitting subface `τ`, prove all vertices of `τ` lie in `coordinateFace (prefixRooms n u.level)`, and prove `τ.vertices.card = u.level`. The new theorem `minimal_section5SegmentSubface_exists_mem_coordinateFace_point_of_vertices_mem_coordinateFace` makes the lower-prefix slice point automatic from the vertex condition, and `minimal_section5SegmentSubface_lowerBoundaryGeometry_of_card_eq_of_vertices_mem_coordinateFace` then packages the result into `Section5MinimalSliceLowerBoundaryGeometry u f`, `Section5MinimalSliceFaceData`, and the predecessor `Section5Step`.
