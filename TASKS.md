...
- [ ] Revisit `Section5PerturbationGenericity.lower_step_exists_of_ne_start` only if it is still needed after the boundary-segment route lands: either derive it from the endpoint package above or explicitly isolate the extra one-cell transversality hypothesis required by the old erased-face contradiction.
