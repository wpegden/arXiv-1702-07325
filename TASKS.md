...
- [ ] Finish `Section5PerturbationGenericity.lower_step_exists_of_ne_start` by proving the two remaining local geometric subgoals for a non-start Section 5 cell `u`: produce a point of `u.cell.realization ∩ coordinateFace (prefixRooms n u.level)` whose image still lies on `prefixBarycenterSegment n u.level` (the new endpoint lemma then forces `prefixBarycenter n u.level` automatically), and strengthen the new room-coverage obstruction to the exact cardinality statement `(section5LowerPrefixVertices u).card = u.level`.
