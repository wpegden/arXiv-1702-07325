...
- [ ] Prove the actual `Section5SimplexSliceGenericity.lower_minimal_slice_face_of_ne_start` field from the manuscript’s segment-intersection picture. The current support layer now has the right target interface: for a non-start cell `u`, choose a minimal segment-hitting subface `τ`, show the generic slice `section5CellSlice u f` is a genuine 1-dimensional segment inside `u.cell`, and deduce that one boundary codimension-one face of `τ` lies in `coordinateFace (prefixRooms n u.level)` with exactly `u.level` vertices. Once that local slice theorem is proved, the new bridge immediately recovers the lower predecessor `Section5Step`.
