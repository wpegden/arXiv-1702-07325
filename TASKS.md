...
- [ ] Prove the actual lower-boundary geometry from the manuscript’s segment-intersection picture. The frontier has sharpened again: for a non-start cell `u`, choose a minimal segment-hitting subface `τ` and construct a codimension-one lower boundary face `ρ ≤ τ` that lies in `coordinateFace (prefixRooms n u.level)`, has `ρ.vertices.card = u.level`, and carries a slice point mapping into `prefixBarycenterSegment n u.level`. The new theorem `minimal_section5SegmentSubface_card_eq_and_vertices_mem_coordinateFace_of_lower_boundary_face` now turns exactly that local lower-boundary-face input into both remaining face-level facts (`τ.vertices.card = u.level` and all vertices of `τ` in the lower prefix face), and `minimal_section5SegmentSubface_lowerBoundaryGeometry_of_lower_boundary_face` then packages the result into `Section5MinimalSliceLowerBoundaryGeometry u f`, `Section5MinimalSliceFaceData`, and the predecessor `Section5Step`.
