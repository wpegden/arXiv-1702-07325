...
- [ ] Prove the actual lower-boundary geometry from the manuscript’s segment-intersection picture. The remaining local target is now sharper: for a non-start cell `u`, choose a minimal segment-hitting subface `τ`, produce one slice point of `τ` lying in `coordinateFace (prefixRooms n u.level)`, and prove the resulting minimal face has `u.level` vertices. The new theorem `minimal_section5SegmentSubface_erase_realization_map_segment_of_mem_coordinateFace_point` then gives all erased-face witnesses automatically, and `minimal_section5SegmentSubface_lowerBoundaryGeometry_of_card_eq_of_mem_coordinateFace_point` packages the result into `Section5MinimalSliceLowerBoundaryGeometry u f`, `Section5MinimalSliceFaceData`, and the predecessor `Section5Step`.
