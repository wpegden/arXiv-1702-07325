...
- [ ] Prove the actual lower-boundary geometry from the manuscript’s segment-intersection picture. The right target is now explicit: for a non-start cell `u`, choose a minimal segment-hitting subface `τ`, show the generic slice `section5CellSlice u f` is a genuine 1-dimensional segment inside `u.cell`, and package its lower endpoint as `Section5MinimalSliceLowerBoundaryGeometry u f`. Once that local slice theorem is proved, the compiled bridge immediately recovers `Section5MinimalSliceFaceData`, the predecessor `Section5Step`, and then `Section5SimplexSliceGenericity.lower_minimal_slice_face_of_ne_start`.
