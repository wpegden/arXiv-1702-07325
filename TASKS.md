...
- [ ] Finish `Section5PerturbationGenericity.lower_step_exists_of_ne_start` by proving the manuscript's actual lower-entry statement for a non-start Section 5 cell `u`. The direct route is now fully reduced to one local contradiction on a canonical object: `section5SegmentSubfaces u f` is a finite family of nonempty subfaces of `u.cell` whose image meets `prefixBarycenterSegment n u.level`, there is a minimal-cardinality such face, and the existing piecewise-affine API extracts an actual point of its realization mapping to the segment. The remaining work is to prove that every vertex of this minimal segment-hitting face lies in `coordinateFace (prefixRooms n u.level)` and that its card is exactly `u.level`; then `exists_section5StartComponentLowerStep_of_subface_card_eq_and_mem_realization_map_segment` yields the predecessor `Section5Step`.
