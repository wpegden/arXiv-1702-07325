...
- [ ] Finish `Section5PerturbationGenericity.lower_step_exists_of_ne_start` by proving the manuscript's actual lower-entry statement for a non-start Section 5 cell `u`. The direct route is now reduced to one concrete local contradiction on a canonical object: `section5SegmentSubfaces u f` is a finite family of nonempty subfaces of `u.cell` whose image meets `prefixBarycenterSegment n u.level`, there is a minimal-cardinality such face, the existing piecewise-affine API extracts an actual point of its realization mapping to the segment, and `IsPiecewiseAffineOn.exists_point_with_nonzero_weights_of_minimal_section5SegmentSubface` upgrades that witness to one whose barycentric weights on the chosen face are all nonzero. The new reduction lemmas `minimal_section5SegmentSubface_erase_not_mem` and `minimal_section5SegmentSubface_vertices_mem_coordinateFace_of_erase_mem` show that the remaining job is now exactly this: if a vertex of the minimal face lies outside `coordinateFace (prefixRooms n u.level)`, prove that erasing that single bad vertex still leaves a face in `section5SegmentSubfaces u f`. Once that one-vertex removal statement is proved, the minimal face lies in the lower prefix face, its card can be upgraded to `u.level`, and `exists_section5StartComponentLowerStep_of_subface_card_eq_and_mem_realization_map_segment` yields the predecessor `Section5Step`.
