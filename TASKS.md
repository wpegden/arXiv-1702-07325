...
- [ ] Continue tying `section5StartNode` to the actual boundary-chain graph data all the way through degree one: use `face_intersection` plus the new boundary-edge convex-hull lemmas to show any two level-1 cells through `e₁` coincide, then deduce the unique level-1 successor and degree one in the actual start component.
