...
- [x] Add the first collapsed-segment support for the stuck Section 5 route: characterize `prefixBarycenterSegment` by `AffineMap.lineMap`, prove any linear map annihilating the segment direction is constant on that segment, prove intersection with the smaller ambient prefix face forces the lower endpoint, choose the explicit collapse map `prefixSegmentCollapseMap`, and package `IsSection5GraphNode.exists_faceHitWitness_eq_under_prefixSegmentCollapse_of_piecewiseAffineOn`, `IsSection5GraphNode.cell_eq_prefixFace_of_incidentFacet`, and `IsSection5GraphNode.exists_point_in_incidentPrefixFace_zeroFiber_of_piecewiseAffineOn`.
