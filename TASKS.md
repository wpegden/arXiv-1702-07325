...
- [x] Add the first collapsed-segment support for the stuck Section 5 route: characterize `prefixBarycenterSegment` by `AffineMap.lineMap`, prove any linear map annihilating the segment direction is constant on that segment, prove intersection with the smaller ambient prefix face forces the lower endpoint, choose the explicit collapse map `prefixSegmentCollapseMap`, and package `IsSection5GraphNode.exists_faceHitWitness_eq_under_prefixSegmentCollapse_of_piecewiseAffineOn` together with `IsSection5GraphNode.cell_eq_prefixFace_of_incidentFacet`.
- [ ] Prove the remaining facet-local collapsed-fiber theorem behind `section5LocalOneComplexGeometry_of_uniqueUpperOrEndpoint`: for a start-component node `v`, use `prefixSegmentCollapseMap n v.level` and `IsSection5GraphNode.exists_faceHitWitness_eq_under_prefixSegmentCollapse_of_piecewiseAffineOn` to show that the zero fiber inside the incident prefix face has at most one upper codimension-one continuation, and that the empty-upper-continuation case already implies `IsSection5Endpoint T f v.1.1`.
